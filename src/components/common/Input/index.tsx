...
const Input: React.FC<IProps> = React.forwardRef<HTMLInputElement, IProps>(
  (
    ...
    const labelClasses = classNames(
      'block w-full mb-1 text-xs font-medium text-gray-500 transition-all duration-200 ease-in-out ',
      {
    ...
    return (
      <div className={wrapperClass}>
        ...
        <input
          ...
          {...props}
        />
        {errorMessage && <p className='text-red-600 text-xs mt-1 w-full md:w-96 max-w-full'>{errorMessage}</p>}
      </div>
    );
  },
);
